...
def find_signals(bis, zss, beichis, klines=None, merged=None):
    ...
    # 二/三类买卖点：去重为「每锚点的前若干有效折返」——缠论中二类是紧接一类后的次低/次高折返、
    # 三类是中枢离开后的首个回抽；此前逐笔判断导致长年累积出上百个冗余信号（噪声），
    # 既失真又拖垮回测读数。现改为：每个一类买卖点取其后的**前 2 支**有效折返笔
    # （R484 扩为 2 支：第 1 支 = 二类、第 2 支 = 类二，见下方依据）；
    # 每个中枢只取其离开后的首个回抽/反抽（不重新进入中枢区间）作为三类。
    # 二类买点：一类买之后，向下折返笔末端价 > 一类买价（次低，不破前低）。取前 2 支。
    # R338: 任一反向折返若破前低(end<=pr0) = 一类买点失效(底背驰被走势否定, 创新低续跌),
    # 该锚点立即终止、不再后扫 —— 原实现破低后继续后扫，会把数笔之后(数十日后)的普通折返
    # 错配成该失效一类的"二类买点"(时序错位+语义误导)。R484 扩大扫描范围时**保留**该终止律。
    # R484: 由「只取首支」改为**累计到第 2 支** —— 第 1 支仍为「二类点」（逐字不变），
    # 第 2 支为「类二点」。依据（5 指数 2021 至今、直接跑生产模块、随机基准 20 日 49.1%）：
    # 一类点之后第 k 个「不破位反向折返」的 20 日胜率 —— 第 1 次 59.3%(买)/62.9%(卖)、
    # **第 2 次 68.2%(买)/73.1%(卖, p=0.011)**、第 3 次退化到 58.8%/59.1%（合并口径
    # 第 2 次 70.8% p=0.002 显著）。⇒ **第 2 次不弱于首个、第 3 次起衰减 ⇒ 只补到第 2 次**。
    # 与段级 R483「段类点」同律（段级：76.0% → 78.3% → 54.5%）—— 两侧同一机制、同一取舍。
    # ★ 反向对照（本轮同时量化）：**三类族**第 2 次起明显退化（笔 73.1%→61.5%→56.4%；
    #   段 81.5%→66.7%→70.6%）⇒ 三类点**不**做同样扩展，两级一致。故本条只扩二类族。
    # ★ 命名让 `kind[:3]` **自带方向**（`类二买`/`类二卖`）；且不含「一类」「三类」子串 ⇒
    #   find_signals 的 `_prio` 归入二类档、attach_rr 走「近程前低」止损 —— 与二类点同族，正确。
    for idx0, pr0 in p1_buys:
        _hit = 0
        for j in range(idx0 + 1, len(bis)):
            bj = bis[j]
            if bj["dir"] != -1:          # 只看向下折返笔（次级别反向）
                continue
            if bj["end_price"] <= pr0:
                # R338: 破前低 = 一类买点被走势否定（底背驰证伪）⇒ 锚点失效、不再后扫
                break
            _hit += 1
            if _hit > 2:
                break                    # 只取到第 2 次（第 3 次起统计上退化）
            signals.append({"bi_index": j,
                            "kind": ("二类买点(次低不破)" if _hit == 1
                                     else "类二买点(二次回抽不破)"),
                            "dir": 1,
                            "date": bj["date_end"], "price": bj["end_price"],
                            "vol_confirm": False, "bc_type": ""})
    # 二类卖点：一类卖之后，向上折返笔末端价 < 一类卖价（次高，不破前高）。同律累计到第 2 支。
    # R338: 对称修复 —— 首支向上反抽若破前高(end>=pr0) = 一类卖点失效, 不再后扫错配二类。
    for idx0, pr0 in p1_sells:
        _hit = 0
        for j in range(idx0 + 1, len(bis)):
            bj = bis[j]
            if bj["dir"] != 1:           # 只看向上反抽笔（次级别反向）
                continue
            if bj["end_price"] >= pr0:
                break                    # R338: 破前高 = 一类卖点证伪 ⇒ 锚点失效
            _hit += 1
            if _hit > 2:
                break
            signals.append({"bi_index": j,
                            "kind": ("二类卖点(次高不破)" if _hit == 1
                                     else "类二卖点(二次反抽不破)"),
                            "dir": -1,
                            "date": bj["date_end"], "price": bj["end_price"],
                            "vol_confirm": False, "bc_type": ""})
...
SEG_BC_ACTIVE_GAP = 2
# R495: 段级背驰「新鲜度」阈值（单位 = **交易日**，端点距末根 ≤ N 根）。
# ★ 与 SEG_BC_ACTIVE_GAP 回答的是**不同**问题，两者不可互换：
#     SEG_BC_ACTIVE_GAP = 结构活跃度（自末段往回数的**段数**）—— 当下还有没有段级背驰在起作用
#     SEG_BC_FRESH_DAYS = 时间新鲜度（端点距末根的**交易日根数**）—— 它是不是刚刚才出现
#   实测可差 90 倍量级：上证 2026-09-18 那条活跃段级底背驰 gap=1（结构上"就在最近一轮"）
#   而 days=44（时间上已是 44 个交易日前）—— 只报 gap 用户会误读成"昨天刚出的"。
# 语义依据：段级背驰刚出现时必然锚在末段且端点≈末根（R487 实测出现日只比段端点晚 1 个
# 交易日），故新鲜度是"这条背驰值不值得今天看"的直接判据。取 5 = 一个交易周。
# ★ 单一来源：主界面 report.py 的「新」标识与雷达 scan_radar.py 的背驰提示**共用本常量**，
#   避免同一语义在两处各写一个数字而静默漂移（R494 的 BC_AREA_RATIO_TH 同款处理）。
SEG_BC_FRESH_DAYS = 5
