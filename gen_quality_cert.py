...
def main():
    ...
    cert = {
        ...
        "accuracy_note": "预测准确性所有安全维度(R70-R88 多轮回测+监控)已到顶/证伪/落地; 数学层面已封顶, 证书仅作可见化与监控; 显示概率p_main校准诚实性由关10监控, 置信带锐度/不确定性校准由关11(audit_interval_score.py)监控, 点预测水平(价位)偏置由关12(audit_point_bias.py)监控, 推演数值内部自洽(存续概率↔带/带嵌套/带有限/文本↔图)由关13(audit_forecast_consistency.py)监控, 推演路径形态保真度(画出的路形状对不对)由关14(audit_path_shape.py)监控——实测逐段方向吻合度≈46~48%(低于naive~50%)、Spearmanρ≈0~-0.2, 即结构主路径的『途中弯法』与真实基本无关, 路径节奏不可用于逐日择时; 极端尾部覆盖(Kupiec POF+下行尾部+最差十分位击穿)由关15(audit_tail_coverage.py)监控——关11无条件IS掩盖的灾难性击穿盲区被定位: 全样本T+30带覆盖与名义5%一致(稳健), 但熊市T+30带例外率33.3%(LRuc=7.1, 拒绝99%)、每3次漏1次(熊市N=9样本偏小, 与关8/关12同源), 印证『暴跌时带过度自信(太窄)』; 仍坚持以带/中位端点为锚、熊市自行加安全垫",
