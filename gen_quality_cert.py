...
def main():
    ...
    cert = {
        ...
        "accuracy_note": "预测准确性所有安全维度(R70-R87 多轮回测+监控)已到顶/证伪/落地; 数学层面已封顶, 证书仅作可见化与监控; 显示概率p_main校准诚实性由关10监控, 置信带锐度/不确定性校准由关11(audit_interval_score.py)监控, 点预测水平(价位)偏置由关12(audit_point_bias.py)监控, 推演数值内部自洽(存续概率↔带/带嵌套/带有限/文本↔图)由关13(audit_forecast_consistency.py)监控, 推演路径形态保真度(画出的路形状对不对)由关14(audit_path_shape.py)监控——实测逐段方向吻合度≈46~48%(低于naive~50%)、Spearmanρ≈0~-0.2, 即结构主路径的『途中弯法』与真实基本无关, 路径节奏不可用于逐日择时, 仍以带/端点为锚",
