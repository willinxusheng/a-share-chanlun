...
def tx_host_count():
    ...
    ★★ **R481 修正(2026-09-18): 必须"跨轮看", 单轮偏斜不构成告警。** 上面那句判据曾把
      09-17 轮(`proxy 12734(79%) / ifzq 1906 / web 1483`, `cold=[]`, `bad={ifzq:18,web:15}`)
      判成"分散未生效", **是错的** —— 两条反证:
        · **同版本对照**: 09-16 同一 `P3b-r28` 实测 **5413 / 5085 / 5109 且 `bad={}`(零失败)**,
          09-17 才偏斜 ⇒ 变量是**源侧状态**, 不是码;
        · **本地实跑**(生产模块 60 票按 seed 均衡抽样): 阶段一 20/20/20、阶段二 30/26/33, 零失败。
      机制: 某轮镜像抖动 → 计数到 `_TX_BAD_PIN` → 60s 冷却 → 冷却期内冷台被 `_tx_order` 排到队尾
      ⇒ 健康台兜底吃下绝大部分流量; 收尾时失败已过冷却窗 ⇒ `cold=[]` 而 `bad` 只剩尾部残留。
      ⇒ 只有"**连续多轮**同一台独大"才是问题(那才叫"该台真坏", 属正常退化, 也不改码)。
...
def tx_host_cold():
    ...
    ★ 判读: 若线上长期只有一台在服务(看 `tx_host_cnt`), 而 `tx_host_cold` 里躺着本来
      **健康**的镜像 ⇒ 说明冷却窗还没走到恢复点(正常, 最坏 60s)。
    ★★ **R481 修正(2026-09-18)**: 原文接着写的"若 `tx_host_cold` 为空却仍单台 ⇒ 才是排序/
      分散逻辑出了问题"**已被证伪, 不要再据此报警** —— 09-17 轮正是 `cold=[]` + 单台 79%,
      但分散逻辑无缺陷(同版本的 09-16 轮 34.7/32.7/32.6% 均衡 + 本地复现 20/20/20)。
      真因: 抖动台在**收尾前**就已过 60s 冷却窗 ⇒ 快照那一刻 `cold` 为空, 但整轮绝大部分
      时间它是被冷却压住的。⇒ **`cold` 是"收尾时刻"的快照, 不能用它反推整轮。**
      跨轮判据见 `tx_host_count` 上方 R481 段。
    """
    with _TX_HOST_BAD_LOCK:
