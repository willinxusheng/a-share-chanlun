...
def forecast_svg(klines, r, wcls, conf, sigma, sym, horizon=60, bt=None, bt_paths=None, breadth_score=None):
    ...
    # 保守改善而非精确标定, 需注明置信局限。
    _KAPPA = {"bull": 1.5, "range": 1.4, "bear": 2.3}  # R171: bull 1.4→1.5 根治 bull T+8 覆盖不足(79.2%→86.8%); range/bear 不变
    _rg = classify_regime(klines)
    _kappa = _KAPPA.get(_rg, 1.4)
