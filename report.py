...
def backtest_table(backtests):
    ...
    return """<h3 class="fc-title">信号历史回测汇总<span class="fc-sub">2021 至今全部买卖点 · 胜率与均收益</span></h3>
      ...
      <tbody>%s</tbody></table>
      <p style="font-size:12px;color:#64748b;margin-top:8px">统计 5 大指数 2021-01 至今全部信号（买点胜=之后涨，卖点胜=之后跌）。买卖点按缠论标准：一类=背驰拐点，二类=次低/次高折返，三类=回抽不进中枢；<b>段一/段二/段三 = 线段级别</b>（段一=线段级背驰拐点，段二=段级一类后次级别首个回抽不破前极值，段三=<b>段级中枢</b>被离开后次级别回抽不重新进入），与笔级<b>分行独立统计、不混计</b>。<b>段三点样本量小</b>（5 指数 2021 至今合计 33 条，其中上证仅 2 条）—— 读数只作方向参考，尤其中短期胜率在 n&lt;15 时易出 100%% 这类假值。样本有限，历史特征，非投资建议。</p>""" % "".join(rows)
...
def main():
    ...
    <section class="panel" id="sec-{sym}">
      ...
      <div style="margin-top:4px;font-size:12px;line-height:1.75;color:#94a3b8">
        ...
        全部漏标。现已补上 <b>段1买·段1卖·段2买·段2卖</b>（R478）与 <b>段3买·段3卖</b>（R480）——
        段级一/二/三类至此齐备，与笔级同构、只换级别。<b>段级中枢由线段序列重构</b>（≥3 条线段重叠区间），
        故段三点的止损锚在<b>段级中枢</b>的下沿/上沿，与笔级三类的止损位不是一回事。
        <b>两级信号各自独立统计</b>（见下方回测表），不混计。</div>
