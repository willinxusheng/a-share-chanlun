...
def forecast_svg(klines, r, wcls, conf, sigma, sym, horizon=60, bt=None, bt_paths=None, breadth_score=None, sent_fc=None):
    ...
    if _bc_fail is not None and _bc_fail.get("price"):
        ...
        # ★ R514: 这条位是**三态**而非常量。R513 只写了「未触及」一态，且用**完成态语气**
        #   （「现价上破 X ⇒ 判据被证伪」）去陈述一个**尚未发生**的条件 ⇒ 易被读成"已经证伪"；
        #   本轮补齐「已上破 / 已跌破」态并附实证。判据与页面同口径 = **收盘价** last 与端点价比较
        #   （不用最高/最低价；两种口径的稳健性见 note 中的并列读数）。
        _broke = (_fp < last) if _bc_fail_kind == "up" else (_fp > last)
        _bmark = (("（<b>已上破</b>）" if _bc_fail_kind == "up" else "（<b>已跌破</b>）")
                  if _broke else "")
        _bc_fail_line = (f'反向证伪位({_ftag}) ≈ <b>{_fp:.0f}</b>{_bmark} · ')
        if _bc_fail_kind == "up":
            if _broke:
                _bc_fail_note = (
                    f"反向证伪位（顶背驰专用）：<b>现价已上破 {_fp:.0f}</b>（{_fd} 顶背驰的高点，"
                    f"高出 <b>{abs(_fdist):.2f}%</b>）⇒ 该顶背驰判据<b>已被证伪</b>"
                    f"（价格创新高且力度未衰竭），「背驰见顶风险」的看跌依据已失效。\n"
                    f"　实证（5 指数 2021 至今 46 个笔级顶背驰 · 前视 20 日 · 收盘价口径，与上句判据同口径）："
                    f"高点被上破 <b>13 例均 -0.76%</b>（上涨占比 38%），未被上破 <b>33 例均 -5.35%</b>"
                    f"（上涨占比 6%）⇒ 相差 <b>4.60pp</b>；放宽为「最高价触及」口径同号"
                    f"（-1.09% vs -5.96%，差 4.88pp）。\n"
                    f"⚠ 两点<b>勿误读</b>：① 判据失效<b>不等于转多</b>（已上破组仍为小幅负收益）；"
                    f"② 情景<b>不会即时切换</b>——引擎要等本背驰移出「最近 3 笔」窗口才重判（实测滞后"
                    f"可达数十个交易日），期间上涨若呈减速形态还会<b>生成新的顶背驰</b>接替，"
                    f"页面可能继续显示看跌，这不算矛盾。\n")
            else:
                _bc_fail_note = (
                    f"反向证伪位（顶背驰专用）：<b>一旦</b>现价上破 {_fp:.0f}（{_fd} 顶背驰的高点，"
                    f"距现价 <b>{_fdist:+.2f}%</b>）⇒ 该顶背驰判据<b>被证伪</b>（创新高且力度未衰竭），"
                    f"「背驰见顶风险」的看跌依据失效。\n"
                    f"⚠ 本假设的两侧条件方向相反、<b>勿混读</b>：上破 {_fp:.0f} 回答「何时推翻本判断」，"
            ...
            if _broke:
                _bc_fail_note = (
                    f"反向证伪位（底背驰专用）：<b>现价已跌破 {_fp:.0f}</b>（{_fd} 底背驰的低点，"
                    f"低出 <b>{abs(_fdist):.2f}%</b>）⇒ 该底背驰判据<b>已被证伪</b>"
                    f"（再创新低且力度未衰竭），「背驰见底机会」的看涨依据已失效。\n"
                    f"　实证（5 指数 2021 至今 52 个笔级底背驰 · 前视 20 日 · 收盘价口径）："
                    f"低点被跌破 <b>25 例均 -2.94%</b>（上涨占比 16%），未被跌破 <b>27 例均 +7.61%</b>"
                    f"（上涨占比 96%）⇒ 相差 <b>10.55pp</b>，底侧的分水岭效应强于顶侧。\n"
                    f"⚠ 该位<b>仅作判据证伪用、不作止损</b>：R489 实测线段端点被跌破后 20 日胜率 "
                    f"53%（p=0.598，与随机无异），据此止损并无统计优势。\n")
            else:
                _bc_fail_note = (
                    f"反向证伪位（底背驰专用）：<b>一旦</b>现价跌破 {_fp:.0f}（{_fd} 底背驰的低点，"
                    f"距现价 <b>{_fdist:+.2f}%</b>）⇒ 该底背驰判据<b>被证伪</b>（再创新低且力度未衰竭），"
                    f"「背驰见底机会」的看涨依据失效。\n"
                    f"⚠ 该位<b>仅作判据证伪用、不作止损</b>：R489 实测线段端点被跌破后 20 日胜率 "
                    f"53%（p=0.598，与随机无异），据此止损并无统计优势。\n")
        # 逼近提醒：距证伪位不足 3% **且尚未突破**时点出（已突破由上面的态覆盖 ⇒ 两态互斥，
        # 否则已上破 1%~3% 时会同时出现「已上破」与「已逼近」，互相矛盾）
        if (not _broke) and abs(_fdist) <= 3.0:
