...
def main():
    ...
    <section class="panel" id="sec-{sym}">
      ...
      <div style="margin-top:6px;font-size:12px;line-height:1.75;color:#64748b">标签：<b>1/2/3买·卖</b> = <b>笔级</b>一类/二类/三类买卖点（背驰拐点 / 次低次高折返 / 回抽不进中枢）·
        ...
        标注点（= 段的最低点）高 <b>2.24%</b> ⇒ 实操应<b>分批建仓</b>（首笔试探），并以「<b>线段端点被有效跌破</b>」
        为<b>证伪位</b>，而不是因为「背驰了」就满仓。<b>样本边界（如实）</b>：买侧仅 30 例 / 卖侧 19 例，
        且 5 指数同期高度共振 ⇒ 独立事件更少，上表胜率只作<b>方向性证据</b>，非统计定论。<br>
        ...
        即此机制；两条旧点仍在引擎里，只是该段端点笔退回未完成）。</div>
