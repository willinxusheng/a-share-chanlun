...
_A_SHARE_MAKEUP = {  # 周末补班（仍交易）
    # 2026 调休补班日（据国务院办公厅《2026 年部分节假日安排》）：01-04(元旦)、
    # 02-14/02-28(春节)、05-09(劳动)、09-20(国庆前)、10-10(国庆后)。
    # 原列表漏登 01-04，已补(R78)；缺失会使该补班日被误当休市，令 _fut 推演时间轴
    # 与 _gap_td 缺口新鲜度护栏少算一个交易日，导致推演落点日期错位。
...
def forecast_svg(klines, r, wcls, conf, sigma, sym, horizon=60, bt=None, bt_paths=None, breadth_score=None):
    ...
    # 覆盖修正 κ（R168 重新标定）：walk-forward 实测(同 audit_forecast_calibration 引擎,
    # ANCHOR_STEP=15, 5指数 N=180/horizon)——κ=1.8 时牛/震荡实测覆盖 96~98%(过宽, 名义90%被系统性
    # 高估, 带几乎无信息量);κ=1.4 时精确命中名义 90%(T+8 89.4%, T+30 91.1%)。早期注释称"κ=1.4→86%/
    # κ=1.8→90.2%"是 R57/R58 改窗口(全历史→近3年)+改中心(中位→均值)之前的旧黄金检验口径, 与当前
    # 公式不复现, 已 stale。故 R168 将 bull/range κ 由 1.8 降至 1.4。κ 用于补偿近窗口经验分位在
    # 5%/95% 极分位的采样误差+长 horizon 非平稳性。√t 缩放假设仍成立(覆盖率随 horizon 分桶均匀)。
    ...
    _KAPPA = {"bull": 1.4, "range": 1.4, "bear": 2.3}  # R168: 1.8→1.4 命中真实~90%覆盖(原1.8实测96~98%过宽)
    ...
    _sp_up = max((_q95 - _q50) * _kappa, 1e-9)     # P95 上沿离散度(近窗口,无额外 regime 放大)
    _sp_dn = max((_q50 - _q05) * _kappa, 1e-9)     # P05 下沿离散度（左尾更宽，如实反映下行风险）
    _sp_up75 = max((_q75 - _q50) * _kappa, 1e-9)
    _sp_dn25 = max((_q50 - _q25) * _kappa, 1e-9)
    ...
    note = (f"主路径失效位：现价有效跌破 ZD {zd:.0f}（收盘确认）→ 主路径失效、风险路径概率上升；风险路径确认需同时满足「跌破 ZD + 周线笔转向下」。\n"
        ...
        f"故<b>上下带非对称</b>——按真实历史经验分位分别给上下沿定宽（替代对称 ±1.645σ 等宽假设）：本指数近 3 年 {horizon} 日对数收益呈右偏，上行离散（P95–P50）约为下行的 1.5–2.5 倍，故<b>上行带更宽</b>，如实容纳单边急涨的肥尾。R57+R58 口径：① 校准窗口由全历史改为<b>近 3 年</b>，剔除 2015 股灾等早期崩溃收益导致的 era-shift 偏悲观；② 中心由中位改为<b>窗口均值（期望）</b>，A 股含正漂移、中位低估中枢，使方向判定正确率由约 36% 升至约 54%；③ 近窗口已含当前波动，<b>不再叠加 regime 因子</b>（此前双重放大使创业板带宽虚胖至 ±60%+）。覆盖修正系数 κ（R168 重新标定）：walk-forward 实测(同回测引擎, 5指数 N=180/horizon)表明 κ=1.8 时牛/震荡实测覆盖达 96~98%(过宽、名义90%被高估、带几乎无信息量)，故 R168 将牛/震荡 κ 由 1.8 降至 1.4，精确命中名义 90%(T+8 89.4%、T+30 91.1%)、各指数均落 89~92%；早期「κ=1.4→86%/κ=1.8→90.2%」系 R57/R58 改窗口与改中心前的旧口径、已 stale。√t 缩放假设仍成立(覆盖率随 horizon 分桶均匀)。<b>κ 按市场环境自适应(R108)</b>：牛/震荡维持 <b>{_KAPPA['range']:.1f}</b>(关15 实证覆盖与名义一致、不退化)，<b>熊市放宽至 {_KAPPA['bear']:.1f}</b>(关15 实证熊市 T+30 原 κ=1.8 漏覆盖 33.3%、LRuc=7.1 拒绝 99%，放宽后给下行富尾补安全垫——A 股熊市下跌更急更肥尾，近 3 年经验分位低估了极端下行)。中线路径为「实测漂移期望（均值）」而非手工情景路径，置信带中线统计诚实；带宽随时间按 √t 扩张（随机游走特性），近月不确定性即已显著，并非线性外推的针状。<b>代价</b>：创业板等超高波动指数 60 日 P05–P95 带宽达 ~119%，这是其真实波动的诚实反映，而非缺陷。\n"
...
def main():
    ...
    calib_note = (
        ...
        "捕捉「行情没动、预测自己跳」的过拟合/数据异常信号（详见 audit_forecast_drift.py）；同时为推演落点日期准确性补全了调休补班日"
        "（_A_SHARE_MAKEUP 原漏登 2026-01-04，已修复，避免 _fut 推演时间轴少算交易日）。两项均为监控/防御层，不改动预测数学；<br>"
